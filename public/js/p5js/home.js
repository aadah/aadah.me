...
class ChromaWanderersCircle {
  ...
  move() {
    ...
    const shift =
      frameCount + Math.pow(1000, this.channel) + Math.pow(10000, this.valence);
    this.x =
      width *
...
class ChromaWanderers extends Transform {
  ...
  setup() {
    ...
    for (let i = 0; i < 3; i += 1) {
      this.circles.push(new ChromaWanderersCircle(radiusMax, i, false));
      this.circles.push(new ChromaWanderersCircle(radiusMax, i, true));
    }
  }
